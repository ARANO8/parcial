...
class LoginModel extends ModeloBasePDO
{
    ...
    {
        $sql = "SELECT email, fullname, password_hash FROM login ";
        $param = array();
    ...
    {
        $sql = "SELECT email, fullname, password_hash FROM login
        WHERE upper(concat(IFNULL(email,''),IFNULL(fullname,''),IFNULL(password_hash,''))) like concat('%',upper(IFNULL(:p_filtro,'')),'%') 
        limit :p_limit
        OFFSET :p_offset  ";
        $param = array();
    ...
    public function register($p_email, $p_fullname, $p_password_hash)
    {
        $sql = "INSERT INTO login(email, fullname, password_hash, registration_date) 
        VALUES (:p_email,:p_fullname,:p_password_hash,now())";
        $param = array();
        array_push($param, [':p_email', $p_email, PDO::PARAM_STR]);
        array_push($param, [':p_fullname', $p_fullname, PDO::PARAM_STR]);
        array_push($param, [':p_password_hash', $p_password_hash, PDO::PARAM_STR]);

        return parent::ginsert($sql, $param);
    }
    public function update($p_email, $p_fullname, $p_password_hash)
    {
    ...
    {
        $sql = "DELETE FROM  login_sis  WHERE  email = :p_email";
        $param = array();
        array_push($param, [':p_email', $p_email, PDO::PARAM_STR]);
